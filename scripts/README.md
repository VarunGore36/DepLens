...
- `evaluate_test_outcomes.py` — test-grounded evaluation: test-outcome labels plus parent-state cases, scored and ablated, same output bundle. `--isolate` uses fresh per-commit venvs with era installs; `--limit` bounds cost. Status: `in progress` (Phase 3/5).
